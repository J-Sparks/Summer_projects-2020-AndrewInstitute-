...
ikdc_modeul_ignorNA <- cbind(ikdc_module_num,ikdc_module_label) %>%  as_tibble()
...
ikdc_modeul2_ignorNA <-ikdc_modeul_ignorNA %>%  dplyr::mutate( ikdc_sum = rowSums(ikdc_modeul_ignorNA[, c(3:19,21)], na.rm=T)) # not included "function_prior_to_your_knee_injury"
skimr::skim(ikdc_modeul2_ignorNA)
...
```{r echo=FALSE}
#ignor NA dataset
ikdc_vari_data_ignorNA <- merge( recode_PRP_data_id2,ikdc_modeul2_ignorNA, by=c("record_id","redcap_event_name"), all.y = T ) 
ikdc_vari_ignorNA <- ikdc_vari_data_ignorNA  %>%  arrange( event_name) %>%
  group_by(event_name,Group ) %>% filter(ikdc_sum != 0)  %>%  # remove na
  dplyr::summarise(ikdc_mean=mean(ikdc_sum, na.rm=T), SD=sd(ikdc_sum, na.rm=T), Min=min(ikdc_sum,na.rm=T), Max=max(ikdc_sum,na.rm=T),Count=n( ))  
 
knitr::kable(ikdc_vari_ignorNA, caption = "Average IKDC Score by Group(ignore NA)") 
...
ikdc_v3 <- ikdc_vari_cbc_group %>% filter( total_injection==3) # included 0
ggplot(ikdc_v3, aes( y= ikdc_sum, x = Group, fill = event_name )) +
  geom_boxplot()
```
...
plot_ikdc_ignorNA <- ikdc_vari_ignorNA %>% ggplot( aes(x= event_name, y= ikdc_mean, color=Group, group=Group)) + 
   geom_line( size=1) + geom_point()+
  geom_text(aes(label = round(ikdc_mean, 2)),
            vjust = 2, hjust = "inward",
            show.legend = TRUE) +
  labs(title = "Average IKDC Score over time by Group(ignore NA)",
...
kks_modeul <- cbind(kks_module_num,kks_module_label) %>% as.tibble() %>% na.omit() #375 to 330
...
# koos_box <- boxplot(kks_modeul2$kks_sum)
# koos_box$out
```
...
## plot by two
kks_v3 <- kks_vari_cbc_group %>% filter( total_injection==3) # included 0
ggplot(kks_v3, aes( y= kks_sum, x = Group, fill = event_name )) +
  geom_boxplot()
...
pot_kks_v2 <- kks_v2  %>% 
  ...
  geom_text(aes(label = round(kks_mean, 2)),
            vjust = 2, hjust = "inward",
            show.legend = TRUE) +
  labs(title = "Average KOOS Score over time by CBC-Group",
       #caption = "1=Baseline, 2=Month 1, 3=Month 3, 4= Month 6, 5=Month 12, 6=Month 18, 7=Month 24",
    x ="Event Time",
  y = "Average KOOS Score") +
    theme(legend.position = "top") +
...
# stst_box <- boxplot(womac_modeul2$womac_sum)
# stst_box$out
...
womac_v3 <- womac_vari_cbc_group %>% filter( total_injection==3) # included 0
ggplot(womac_v3, aes( y= womac_sum, x = Group, fill = event_name )) +
  geom_boxplot()
...
plot_womac_v2 <- womac_v2  %>% ggplot( aes(x= event_name, y= womac_mean, color=Group, group=Group)) + 
  ...
  geom_text(aes(label = round(womac_mean, 2)),
            vjust = 2, hjust = 0.5,
            show.legend = TRUE) +
  labs(title = "Average WOMAC Score over time by CBC-Group",
     x ="Event Time",
  y = "Average WOMAC Score") +
  theme( legend.position = "top" ) +
  ylim(20,44)
